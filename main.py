...
# Modifiez cette liste selon vos besoins. Si la liste est vide [], le bot envoie TOUT.
...
def send_discord(title, link, description):
    ...
    # Nettoyage du titre pour générer des requêtes de recherche propre
    search_query = quote(title.replace("🚨", "").strip())
    
    # Liens de recherche directe sur les réseaux
...
def main():
    ...
    for item in items:
        ...
        if matches_keywords(full_text):
            ...
            count += 1
            if count >= 3:  # Envoie 3 projets max par exécution
                break
